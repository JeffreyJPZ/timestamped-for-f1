...
type Cache interface {
	// Lookup returns a pair of values where the first value is the value associated with the key if there was a cache hit,
	// or nil if the key was not found. The second value indicates whether there was an error.
	Lookup(ctx context.Context, key string) (any, error)
